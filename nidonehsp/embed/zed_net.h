...
//  if the socket is non-blocking, then this can return 1 if the socket isn't ready
//  returns -1 otherwise. (call 'zed_net_get_error' for more info)
ZED_NET_DEF int zed_net_tcp_socket_receive(zed_net_socket_t *remote_socket, int* rcv_size, void *data, int size);
...
// -1 for error
// 0 for not ready socket, or ready socket but no data
// 1 connected and any data to receive
ZED_NET_DEF int zed_net_check_connection(zed_net_socket_t *socket);
ZED_NET_DEF int zed_net_check_connection_timeout(zed_net_socket_t *socket, int timeout_usec);
...
ZED_NET_DEF int zed_net_udp_socket_open(zed_net_socket_t *sock, unsigned int port, int non_blocking) {
    ...
    if (non_blocking) {
#ifdef _WIN32
        u_long tnb = (u_long)non_blocking;
        if (ioctlsocket(sock->handle, FIONBIO, &tnb) != 0) {
            zed_net_socket_close(sock);
            return zed_net__error("Failed to set socket to non-blocking");
        }
        non_blocking = (int)tnb;
#else
...
ZED_NET_DEF int zed_net_tcp_socket_receive(zed_net_socket_t *remote_socket, int* rcv_size, void *data, int size) {
    ...
    if (rcv_size) { *rcv_size = received_bytes; }
    return 0;
}

ZED_NET_DEF int zed_net_check_connection(zed_net_socket_t *socket) {
    return zed_net_check_connection_timeout(socket, 0);
}
...
ZED_NET_DEF int zed_net_check_connection_timeout(zed_net_socket_t *socket, int timeout_usec) {
    ...
    FD_ZERO(&readfd);
    FD_SET((SOCKET)(socket->handle), &readfd);

    //FD_ZERO(&writefd);
    //FD_SET(remote_socket->handle, &writefd);

    timer.tv_sec = 0;
    timer.tv_usec = timeout_usec < 0 ? 0 : (long)timeout_usec;

    retval = select(0, &readfd, NULL, NULL, &timer);
    if (retval == 0)
        return 0;// not ready
    else if (retval == SOCKET_ERROR) {
        return zed_net__error("Got socket error from select()");
    ...
    //if (!FD_ISSET(remote_socket->handle, &writefd)) { return 2; }
    return FD_ISSET(socket->handle, &readfd) ? 1 : 0;
}
