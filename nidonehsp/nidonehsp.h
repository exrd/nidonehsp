...
#define N2_ASIN(x)			asin(x)
#define N2_ACOS(x)			acos(x)
...
#define N2_ASINF(x)			asinf(x)
#define N2_ACOSF(x)			acosf(x)
...
#ifndef N2_SIND
#define N2_SIND				N2_SIN
#endif
#ifndef N2_COSD
#define N2_COSD				N2_COS
#endif
#ifndef N2_TAND
#define N2_TAND				N2_TAN
#endif
#ifndef N2_ASIND
#define N2_ASIND			N2_ASIN
#endif
#ifndef N2_ACOSD
#define N2_ACOSD			N2_ACOS
#endif
#ifndef N2_ATAN2D
#define N2_ATAN2D			N2_ATAN2
#endif
#ifndef N2_SQRTD
#define N2_SQRTD			N2_SQRT
#endif
#ifndef N2_POWD
#define N2_POWD				N2_POW
#endif
#ifndef N2_EXPD
#define N2_EXPD				N2_EXP
#endif
#ifndef N2_LOGD
#define N2_LOGD				N2_LOG
#endif
#ifndef N2_FMODD
#define N2_FMODD			N2_FMOD
#endif
#ifndef N2_ROUNDD
#define N2_ROUNDD			N2_ROUND
#endif
#ifndef N2_FLOORD
#define N2_FLOORD			N2_FLOOR
#endif
#ifndef N2_CEILD
#define N2_CEILD			N2_CEIL
#endif
...
#define N2_VERSION		(0x00000104)// reserved(1byte)-major(1byte)-minor(1byte)-patch(1byte)
...
#define N2_CLAMP(x, l, h)		((x) < (l) ? (l) : (x) > (h) ? (h) : (x))
...
//=============================================================================
// ベクトル
#define N2_VEC2_DECLARE(prefix, type) \
	typedef union n2_##prefix##vec2_t n2_##prefix##vec2_t; \
	union n2_##prefix##vec2_t \
	{ \
		struct { type x_, y_; }; \
		type e_[2]; \
	}; \
...
N2_VEC2_DECLARE(f, float);
N2_VEC2_DECLARE(d, double);
#undef N2_VEC2_DECLARE

#define N2_VEC3_DECLARE(prefix, type) \
	typedef union n2_##prefix##vec3_t n2_##prefix##vec3_t; \
	union n2_##prefix##vec3_t \
	{ \
		struct { type x_, y_, z_; }; \
		struct { type r_, g_, b_; }; \
		n2_##prefix##vec2_t xy_; \
		n2_##prefix##vec2_t rg_; \
		type e_[3]; \
	}; \
	 \
...
N2_VEC3_DECLARE(f, float);
N2_VEC3_DECLARE(d, double);
#undef N2_VEC3_DECLARE

#define N2_VEC4_DECLARE(prefix, type) \
	typedef union n2_##prefix##vec4_t n2_##prefix##vec4_t; \
	union n2_##prefix##vec4_t \
	{ \
		struct { type x_, y_, z_, w_; }; \
		struct { type r_, g_, b_, a_; }; \
		struct { n2_##prefix##vec2_t xy_, zw_; }; \
		n2_##prefix##vec3_t xyz_; \
		n2_##prefix##vec3_t rgb_; \
		type e_[4]; \
	}; \
	 \
	...
	N2_API n2_##prefix##vec4_t n2_##prefix##vec4_mul(const n2_##prefix##vec4_t lhs, const n2_##prefix##vec4_t rhs); \
...
N2_VEC4_DECLARE(f, float);
N2_VEC4_DECLARE(d, double);
#undef N2_VEC4_DECLARE

#define N2_QUAT4_DECLARE(prefix, type) \
	typedef union n2_##prefix##quat4_t n2_##prefix##quat4_t; \
	union n2_##prefix##quat4_t \
	{ \
		struct { type x_, y_, z_, w_; }; \
		struct { type r_, g_, b_, a_; }; \
		n2_##prefix##vec3_t xyz_; \
		n2_##prefix##vec4_t xyzw_; \
		type e_[4]; \
	}; \
	 \
...
N2_QUAT4_DECLARE(d, double);
#undef N2_QUAT4_DECLARE

// column major
#define N2_MAT4_DECLARE(prefix, type) \
	typedef union n2_##prefix##mat4_t n2_##prefix##mat4_t; \
	union n2_##prefix##mat4_t \
	{ \
		struct { n2_##prefix##vec4_t x_, y_, z_, w_; }; \
		n2_##prefix##vec4_t col_[4]; \
		type m_[4][4]; \
		type e_[16]; \
	}; \
	 \
...
N2_MAT4_DECLARE(f, float);
N2_MAT4_DECLARE(d, double);
#undef N2_MAT4_DECLARE
...
{
	N2H_FILESYSTEM_DIRECTORY_OPERATION_MAKE = 0,
...
typedef struct n2h_filesystem_filestat_t n2h_filesystem_filestat_t;
struct n2h_filesystem_filestat_t
{
	n2_bool_t is_directory_;
	size_t filesize_;
	size_t attributes_;
...
typedef struct n2h_filesystem_statparam_t n2h_filesystem_statparam_t;
struct n2h_filesystem_statparam_t
{
	n2_state_t* state_;
	const char* filepath_;
	n2h_filesystem_filestat_t* dst_filestat_;
	n2h_filesystem_t* filesystem_;
};

typedef struct n2h_filesystem_removeparam_t n2h_filesystem_removeparam_t;
struct n2h_filesystem_removeparam_t
{
	n2_state_t* state_;
	const char* filepath_;
	n2_bool_t allow_file_;
	n2_bool_t allow_dir_;
	n2h_filesystem_t* filesystem_;
};
...
typedef struct n2h_filesystem_dirparam_t n2h_filesystem_dirparam_t;
struct n2h_filesystem_dirparam_t
{
	n2_state_t* state_;
	n2h_filesystem_directory_operation_e operation_;
	const char* dirpath_;
	n2h_filesystem_t* filesystem_;
};
...
struct n2h_filesystem_t
{
	...
	int priority_;
	int id_;
	n2h_filesystem_stat_func_t stat_func_;
	n2h_filesystem_remove_func_t remove_func_;
